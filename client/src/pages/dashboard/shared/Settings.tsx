      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '30px' }}>
        ...
        <div className="glass-card" style={{ padding: '30px', borderRadius: '24px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px' }}>
            {[{ id: 'light', icon: <Sun size={20} />, label: 'Light' }, { id: 'dark', icon: <Moon size={20} />, label: 'Dark' }, { id: 'system', icon: <Monitor size={20} />, label: 'System' }].map(t => (
              <button key={t.id} onClick={() => setTheme(t.id)} style={{ flex: 1, padding: '15px', borderRadius: '12px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', border: `2px solid ${theme === t.id ? 'var(--primary)' : 'var(--border)'}`, backgroundColor: theme === t.id ? 'rgba(37, 99, 235, 0.05)' : 'white', cursor: 'pointer', transition: '0.2s' }}>
                {t.icon}
